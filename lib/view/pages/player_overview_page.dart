...
class _PlayerOverviewPageState extends State<PlayerOverviewPage> {
  ...
  void editProperties(Player p) {
    ...
    Map<String, dynamic> changedProperties = Map<String, dynamic>.from(
      properties,
    );
    ...
    properties.forEach((key, value) {
      propertyWidgets.add(
        TextField(
          ...
          decoration: InputDecoration(
            border: OutlineInputBorder(),
            labelText: '${key.tr()} (${defaultProperties[key]})',
          ),
        ),
      );
      propertyWidgets.add(const SizedBox(height: 12.0));
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('player_overview').tr()),
      body: ListView(
